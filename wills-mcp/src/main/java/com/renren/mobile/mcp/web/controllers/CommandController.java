/**
 * $Id: CommandController.java 4870 2013-03-07 07:46:56Z ji.sun $
 * Copyright 2009-2012 Oak Pacific Interactive. All rights reserved.
 */
package com.renren.mobile.mcp.web.controllers;
...
import com.renren.mobile.mcp.api.entity.McpResponse;
import com.renren.mobile.mcp.api.entity.MobileClientAppInfo;
import com.renren.mobile.mcp.api.entity.RequestBaseContext;
...
public class CommandController extends AbstractController implements InitializingBean {
    ...
    protected ModelAndView handleRequestInternal(HttpServletRequest request,
        ...
        try {
            ...
            ApiCommandContext context = new ApiCommandContext(requestBaseContext.getAppInfo(),
                    ...
                    binaryParams, requestBaseContext.getTicket(), requestBaseContext.getSecretKey());
            ...
            String httpAccessLogMsg = String.format(httpAccessLogFormat//
                    ...
                    , StringUtils.defaultString((String) request.getParameter(HttpConstants.PARAM_CLIENT_INFO)) // clientInfo
            );
            httpAccessLogger.info(httpAccessLogMsg);
            // stdlog
            logger.info(httpAccessLogMsg);
        }
        return null;
    }
    ...
    private boolean validateBaseRequiredParams(RequestBaseContext requestBaseContext,
        ...
        String appSecretKey = requestBaseContext.getAppInfo().getSecretKey();
        if (StringUtils.isNotEmpty(t)) {
            // 有ticket才有userId
            UserPassport userPassport = passportService.getPassportByTicket(t);
            if (userPassport != null && userPassport.getUserId() != 0
                    && t.equals(userPassport.getTicket())) {
                requestBaseContext.setUserId(userPassport.getUserId());
                requestBaseContext.setSecretKey(appSecretKey + userPassport.getUserSecretKey());
            ...
            requestBaseContext.setSecretKey(appSecretKey);
        }
    ...
    private boolean validateRequestFrequency(RequestBaseContext requestBaseContext,
        ...
        String sig = requestParamMap.get(HttpConstants.PARAM_SIG);
        try {
            String cacheSig = sigCache.get(sig);
            if (logger.isDebugEnabled()) {
            ...
            if (cacheSig != null) { // 是重发的请求，用户访问过于频繁
                ...
                sigCache.set(sig, "1", sigCacheTimeout);
                return true;
            }
        } catch (Throwable t) {
    ...
    public void setSigCache(SigCache sigCache) {
