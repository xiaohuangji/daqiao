...
public interface GuideEventDAO {
	...
	@SQL("insert into guide_event values (:1.guideId,:1.userId,:1.eventId,:1.status,:1.satisfaction,:1.createTime,:1.eventType)")
	...
	@SQL("update guide_event set satisfaction=:2 where event_id=:1 and status=1")
	public int setSatisfaction(long eventId,int satisfaction,int guideId);
	
	@SQL("select * from guide_event where guide_id=:1 order by createTime limit :2,:3")
